<template>
  <div class="space-y-6">
    <div>
      <h1 class="text-3xl font-black text-gray-900">Points de Bienvenue</h1>
      <p class="text-gray-600 mt-2">Configurez les points offerts aux nouveaux clients</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      <div class="flex items-start justify-between mb-6">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="text-xl font-bold text-gray-900">Configuration des Points de Bienvenue</h2>
            <p class="text-sm text-gray-600">Offrez automatiquement des points aux nouveaux inscrits</p>
          </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-3xl font-bold text-gray-900 mt-2">{{ stats.totalApplied }}</p>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-gray-600">Points totaux</p>
            <p class="text-3xl font-bold text-gray-900 mt-2">{{ stats.totalPoints.toLocaleString() }}</p>
            <p class="text-xs text-gray-500 mt-1">Points offerts</p>
          </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
      <div class="p-6 border-b border-gray-200">
        <h3 class="text-lg font-bold text-gray-900">Derniers bonus de bienvenue appliqués</h3>
      </div>
      
      <div v-if="recentBonuses.length === 0" class="p-8 text-center text-gray-500">
        ...
        <table class="w-full">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Points offerts</th>
              <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Date</th>
              <th class="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Appliqué par</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200">
            <tr v-for="bonus in recentBonuses" :key="bonus.id" class="hover:bg-gray-50">
              <td class="px-6 py-4">
                <div class="flex items-center gap-2">
                  ...
                  <div>
                    <p class="text-sm font-medium text-gray-900">{{ bonus.customer_name }}</p>
                    <p class="text-xs text-gray-500">{{ bonus.customer_email }}</p>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center gap-1">
                  ...
                  <span class="font-semibold text-gray-900">{{ bonus.points_awarded }}</span>
                </div>
              </td>
